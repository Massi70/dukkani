...
class Usermodel extends CI_Model {
	...
	public function checkFbId($fb_id){
		$sql = $this->db->get_where("users",array("fb_id"=>$fb_id));
		 $res = $sql->num_rows();
		 return $res;
	}

	public function checkGmailId($gmail_id){
		$sql = $this->db->get_where("users",array("gmail_id"=>$gmail_id));
		 $res = $sql->num_rows();
		 return $res;
	}
   ...
   function saveAddress($params){
			$insert = array(
						...
						"villa_address"=>$params['villa_address'],
	...
	function getAllProducts($store_id){
		$this->db->select('p.id, p.store_id, p.category_id, p.arabic_name, p.english_name,CONCAT("'.base_url().'uploads/products/",image) as image,
		 p.description, p.price, p.color, s.size,f,flavor, p.is_active, p.create_date');
		$this->db->join('sizes s', 's.id = p.size_id','left');
		$this->db->join('flavors f', 'f.id = p.flavor_id','left');
		$this->db->where(array('p.is_active'=>'1','p.store_id'=>$store_id));
	...
	function getProductsByCategory($category_id,$language,$store_id){
		/*if($language == 'arabic'){
		...
		$this->db->join('sizes s', 's.id = p.size_id','left');
		$this->db->join('flavors f', 'f.id = p.flavor_id','left');
		$this->db->where(array('p.is_active'=>'1', 'p.category_id'=>$category_id,'p.store_id'=>$store_id));
		$sql = $this->db->get('products p');
		$data=$sql->result_array();    // fetches record from the stores tables
	  	return 	$data;
	...
	function search($search=''){
		if($search != ''){
			$this->db->select('id, store_id, category_id, arabic_name, english_name,CONCAT("'.base_url().'uploads/products/",image) as image,
			 description, price, size_id, flavor_id,color, is_active, create_date');
	...
	function myOrders($user_id,$language){
		if($language == 'arabic'){
			...
			$this->db->select('o.id as orderId,o.order_number,o.user_id,o.product_id,o.quantity,o.product_price,o.price_per_quantity,
				...
				p.description,p.price,s.size,p.color,f.flavor,order_status,f.flavor,p.is_active,DATE(o.create_date) AS OrderDate');
		}
	...
	function orderDetails($order_number){
		$this->db->select('o.id,o.order_number,o.user_id,o.product_id,p.store_id,s.store_name,
			p.arabic_name,p.english_name,o.quantity,o.product_price,
			CONCAT("'.base_url().'uploads/products/",p.image) as image,
			o.price_per_quantity,o.size,o.street_address,o.villa_address,o.flavor,o.order_status,DATE(o.create_date) as OrderDate');
		$this->db->join('products p', 'p.id = o.product_id','left');
   ...
   function changePassword($user_id,$data){
    ...
    public function feedback($data){	
		$this->db->insert('feedback', $data);	// insert data into `users` table
		return $id = $this->db->insert_id();	
